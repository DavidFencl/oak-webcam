...
`tests/test_pointcloud.py` verifies occlusion, clipping, fixed view, metric target projection, bounded sampling and invalid settings without hardware. Synthetic renderer evidence belongs under `evidence/`; it proves projection behavior only. Live NAS point-cloud video delivery still requires an OAK4 run and a decoded output frame. Holistic replay is pending for this standalone USB path.
