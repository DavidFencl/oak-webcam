...
Mock browser tests exercised preset switching and custom-path submission at desktop/mobile widths without JavaScript errors. A read-only real browser session displayed live point-cloud preview and current status. No access token is included in shared documentation.
