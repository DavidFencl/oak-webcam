...
- Date: 2026-09-10
- Status: Development build and live USB capture verified; consumer app checks and distributable package pending. See ../validation-2026-09-10.md.
- Brief: ../brief.md

## First demo

Prove one customizable video output appears as a USB camera on the host. Start with RGB, 3840x2160 at 30 FPS, MJPEG, matching the reference. Defer audio, dynamic modes, graphical editing and simultaneous applications.
...
The user edits Python pipeline.py. Exactly one selected image output feeds the webcam, even if the internal pipeline has other branches. The first contract requires 3840x2160 NV12 at 30 FPS. Depth/detections must be rendered into a compatible image before selection.
